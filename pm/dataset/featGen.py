...
class FeatureProcesser:
    ...
    def scale_feat(self, data):
        ...
        if self.config.enable_cov_features:
            ...
            for idx in range(self.config.cov_lookback, datax['date'].nunique()):
                ...
                sigPeriodReturn = np.log(sigPeriodClose / sigPeriodClose.shift(1)).dropna()
    ...
    def load_all_data_dict(self):
        ...
        # data['extra_valid'] = {}
        # data['extra_test'] = {}
        a1 = pd.read_csv(f'/root/quant-ml-qlib/MADCSP/datasets/{self.config.csp.lower()}/madcsp/bftrain.csv')[['date','stock','DAILYRETURNS-21']]
        a2 = pd.read_csv(f'/root/quant-ml-qlib/MADCSP/datasets/{self.config.csp.lower()}/madcsp/train.csv')[['date','stock','open','close','DAILYRETURNS-21','MA-21']]
        a3 = pd.read_csv(f'/root/quant-ml-qlib/MADCSP/datasets/{self.config.csp.lower()}/madcsp/valid.csv')[['date','stock','open','close','DAILYRETURNS-21','MA-21']]
        a4 = pd.read_csv(f'/root/quant-ml-qlib/MADCSP/datasets/{self.config.csp.lower()}/madcsp/test.csv')[['date','stock','open','close','DAILYRETURNS-21','MA-21']]
        data['all_data'] = pd.concat([a1,a2,a3,a4])

        m1 = pd.read_csv(f'/root/quant-ml-qlib/MADCSP/datasets/{self.config.csp.lower()}/madcsp/extra_train_fine_market.csv')
        b1 = pd.read_csv(f'/root/quant-ml-qlib/MADCSP/datasets/{self.config.csp.lower()}/madcsp/extra_train_fine_stock.csv')
        m2 = pd.read_csv(f'/root/quant-ml-qlib/MADCSP/datasets/{self.config.csp.lower()}/madcsp/extra_valid_fine_market.csv')
        b2 = pd.read_csv(f'/root/quant-ml-qlib/MADCSP/datasets/{self.config.csp.lower()}/madcsp/extra_valid_fine_stock.csv')
        m3 = pd.read_csv(f'/root/quant-ml-qlib/MADCSP/datasets/{self.config.csp.lower()}/madcsp/extra_test_fine_market.csv')
        b3 = pd.read_csv(f'/root/quant-ml-qlib/MADCSP/datasets/{self.config.csp.lower()}/madcsp/extra_test_fine_stock.csv')
        data['extra_all_data']['fine_market'] = pd.concat([m1,m2,m3])
        data['extra_all_data']['fine_stock'] = pd.concat([b1,b2,b3])

        self.techIndicatorLst = pd.read_csv(f'/root/quant-ml-qlib/MADCSP/datasets/{self.config.csp.lower()}/madcsp/techIndicatorLst.csv')['0'].tolist()
        return data
