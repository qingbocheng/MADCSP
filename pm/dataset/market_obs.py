...
class MarketObserver:
    def __init__(self, config, action_dim):
        ...
        if isParallel:
            self.optimizer = optim.AdamW(self.mkt_obs_model.module.parameters(), lr=self.config.po_lr, weight_decay=self.config.po_weight_decay)
        else:
            self.optimizer = optim.AdamW(self.mkt_obs_model.parameters(), lr=self.config.po_lr, weight_decay=self.config.po_weight_decay)
    ...
    def train(self, **label_kwargs):
        ...
        patience = 6
        threshold = 1e-5
        if len(self.mkt_loss) > patience:
            recent_losses = self.mkt_loss[-patience:]
            diffs = [abs(recent_losses[i] - recent_losses[i - 1]) for i in range(1, len(recent_losses))]
        
            if all(diff < threshold for diff in diffs):
                print(f"Early stopping triggered! Last {patience} loss diffs < {threshold}.")
        ...
        print(disp_str)
        self.mkt_loss.append(loss_val.detach().cpu().item())
        self.reset()
    ...
    def update_hidden_vec_reward(self, mode, rate_of_price_change, mkt_direction):
        ...
        a=Huber((rate_of_price_change-1),last_hidden_vec).view(-1)
        self.hidden_vector_reward_lst.append(a) #
...
class MLP_1(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        feat_s_length = self.config.topK * 102
